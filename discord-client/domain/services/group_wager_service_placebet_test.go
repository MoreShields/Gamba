...
func findParticipantInScenario(participants []*entities.GroupWagerParticipant, userID int64) *entities.GroupWagerParticipant {
...
func TestGroupWagerService_PlaceBet_TableDriven(t *testing.T) {
	...
	testCases := []struct {
		...
		validate  func(*testing.T, *entities.GroupWagerParticipant, error)
	}{
		{
			...
			betAmount: 1000,
			betOption: 0,
			validate: func(t *testing.T, participant *entities.GroupWagerParticipant, err error) {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name != "insufficient balance" {
				...
				if existingParticipant != nil {
					...
					fixture.Mocks.GroupWagerRepo.On("SaveParticipant", fixture.Ctx, mock.MatchedBy(func(p *entities.GroupWagerParticipant) bool {
				...
				if tc.wagerType == entities.GroupWagerTypePool {
