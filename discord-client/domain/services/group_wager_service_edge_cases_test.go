...
func TestGroupWagerService_EdgeCases(t *testing.T) {
	...
	t.Run("wager state validation", func(t *testing.T) {
		invalidStates := []struct {
			state         entities.GroupWagerState
			expectedError string
		}{
			{entities.GroupWagerStateResolved, "cannot place bets on resolved wager"},
			{entities.GroupWagerStateCancelled, "cannot place bets on cancelled wager"},
		}
	...
	t.Run("creator vs resolver permissions", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.shouldSucceed {
					fixture.Mocks.GroupWagerRepo.On("Update", fixture.Ctx, mock.MatchedBy(func(w *entities.GroupWager) bool {
						return w.State == entities.GroupWagerStateCancelled
					})).Return(nil)
