...
// Accept transitions the wager to voting state
func (w *Wager) Accept() {
	if w.State == WagerStateProposed {
		w.State = WagerStateVoting
		now := time.Now()
		w.AcceptedAt = &now
	}
}

// Decline transitions the wager to declined state
func (w *Wager) Decline() {
	if w.State == WagerStateProposed {
		w.State = WagerStateDeclined
	}
}

// Resolve transitions the wager to resolved state with a winner
func (w *Wager) Resolve(winnerID int64) {
	if w.State == WagerStateVoting && w.IsParticipant(winnerID) {
		w.State = WagerStateResolved
		w.WinnerDiscordID = &winnerID
		now := time.Now()
		w.ResolvedAt = &now
	}
}