...
func (h *MockHelper) ExpectUserLookup(discordID int64, user *models.User) {
...
func (h *MockHelper) ExpectNewParticipant(args ...interface{}) {
	if len(args) == 1 {
		...
		participant := args[0].(*models.GroupWagerParticipant)
		...
		h.mocks.GroupWagerRepo.On("SaveParticipant", mock.Anything, mock.MatchedBy(func(p *models.GroupWagerParticipant) bool {
...
func (h *MockHelper) ExpectBalanceHistoryRecord(history *models.BalanceHistory) {
	h.mocks.BalanceHistoryRepo.On("Record", mock.Anything, mock.MatchedBy(func(h *models.BalanceHistory) bool {
...
func (h *MockHelper) ExpectBalanceHistoryRecordSimple(discordID int64, balanceAfter int64, transactionType models.TransactionType) {
	h.mocks.BalanceHistoryRepo.On("Record", mock.Anything, mock.MatchedBy(func(h *models.BalanceHistory) bool {
...
type GroupWagerScenario struct {
	...
	userMap      map[int64]*models.User // internal map for quick user lookup
}

// GetUser retrieves a user by discord ID from the scenario
func (s *GroupWagerScenario) GetUser(discordID int64) (*models.User, bool) {
	if s.userMap == nil {
		s.buildUserMap()
	}
...
func (s *GroupWagerScenario) buildUserMap() {
	s.userMap = make(map[int64]*models.User)
...
func (s *GroupWagerScenario) WithPoolWager(resolverID int64, condition string) *GroupWagerScenario {
	s.Wager.WagerType = models.GroupWagerTypePool
...
func (s *GroupWagerScenario) WithOptions(options ...interface{}) *GroupWagerScenario {
	s.Options = make([]*models.GroupWagerOption, 0, len(options))
	for i, opt := range options {
		switch v := opt.(type) {
		case string:
			// Create option from string
			s.Options = append(s.Options, &models.GroupWagerOption{
				ID:             int64(i + 1),
...
func (s *GroupWagerScenario) WithHouseWager(resolverID int64, condition string) *GroupWagerScenario {
	s.Wager.WagerType = models.GroupWagerTypeHouse
	s.Wager.ResolverDiscordID = &resolverID
	s.Wager.Condition = condition
	return s
}
