...
func TestUserMetricsService_GetLOLPredictionStats(t *testing.T) {
	...
	t.Run("calculates accuracy for LOL predictions correctly", func(t *testing.T) {
		...
		mockWagerRepo := new(MockWagerRepository)
		...
		predictions := []*models.GroupWagerPrediction{
...
func TestUserMetricsService_GetWagerPredictionStats(t *testing.T) {
	...
	t.Run("calculates stats for all wagers when no filter", func(t *testing.T) {
		...
		mockGroupWagerRepo.On("GetGroupWagerPredictions", ctx, (*models.ExternalSystem)(nil)).Return(predictions, nil)
...
func TestUserMetricsService_GetScoreboard(t *testing.T) {
	...
	t.Run("returns scoreboard with correct statistics", func(t *testing.T) {
		...
		// Mock active wagers
		activeWagers1 := []*models.Wager{{ID: 1}, {ID: 2}}
		activeWagers2 := []*models.Wager{{ID: 3}}
		...
		// Mock wager stats
		wagerStats1 := &models.WagerStats{TotalResolved: 10, TotalWon: 6}
		wagerStats2 := &models.WagerStats{TotalResolved: 5, TotalWon: 2}
		...
		// Mock bet stats
		betStats1 := &models.BetStats{TotalBets: 20, TotalWins: 15}
		betStats2 := &models.BetStats{TotalBets: 10, TotalWins: 3}
	...
	t.Run("applies limit correctly", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			mockWagerRepo.On("GetActiveByUser", ctx, int64(100+i)).Return([]*models.Wager{}, nil)
			mockWagerRepo.On("GetStats", ctx, int64(100+i)).Return(&models.WagerStats{}, nil)
			mockBetRepo.On("GetStats", ctx, int64(100+i)).Return(&models.BetStats{}, nil)
		}
...
func TestUserMetricsService_GetUserStats(t *testing.T) {
	...
	t.Run("returns complete user stats", func(t *testing.T) {
		...
		groupWagerStats := &models.GroupWagerStats{
			TotalGroupWagers: 5,
			TotalProposed:    2,
			TotalWon:         3,
			TotalWonAmount:   1500,
