...
func TestGroupWagerService_PlaceBet_WithLolLimits(t *testing.T) {
	...
	testCases := []struct {
		name          string
		setupWager    func() *models.GroupWagerDetail
		betAmount     int64
		expectError   bool
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.expectError {
				...
				// Mock the save
				mockGroupWagerRepo.On("SaveParticipant", ctx, mock.AnythingOfType("*models.GroupWagerParticipant")).
					Return(nil)
				mockGroupWagerRepo.On("UpdateOptionTotal", ctx, int64(1), tc.betAmount).
					Return(nil)
				mockGroupWagerRepo.On("Update", ctx, mock.AnythingOfType("*models.GroupWager")).
					Return(nil)
			}
...
func TestGroupWagerService_PlaceBet_DifferentMaxLimits(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Set up LoL wager
			wagerDetail := &models.GroupWagerDetail{
				Wager: &models.GroupWager{
					ID:      1,
					GuildID: TestGuildID,
					State:   models.GroupWagerStateActive,
