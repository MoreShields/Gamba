...
func (h *LoLHandlerImpl) createHouseWagerForGuild(
	...
	votingPeriodMinutes := 30 // 30 minutes for betting
	...
	channelID := int64(0)
	if guildSettings.PrimaryChannelID != nil {
		channelID = *guildSettings.PrimaryChannelID
	}
	...
	// Post to Discord
	if err := h.discordPoster.PostHouseWager(ctx, postDTO); err != nil {
		log.WithFields(log.Fields{
			"guild":   guild.GuildID,
			"wagerID": wagerDetail.Wager.ID,
			"error":   err,
...
func (h *LoLHandlerImpl) resolveHouseWager(ctx context.Context, guildID, wagerID int64, won bool) error {
	...
	// Use system user ID (0) for resolution
