...
class LoLTrackerService:
            ...
            try:
                ...
                for player in tracked_players:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error polling player {player.summoner_name}#{player.region}: {e}"
                        )
    ...
    async def _poll_player_game_state(self, player):
        ...
        if not player.summoner_id:
            logger.warning(
                f"Player {player.summoner_name}#{player.region} has no summoner_id, skipping"
            )
            return
        ...
        async with self.db_manager.get_session() as session:
            ...
            if previous_status != current_status:
                logger.info(
                    f"Game state changed for {player.summoner_name}#{player.region}: "
    ...
    async def _get_riot_game_state(self, player) -> Optional[CurrentGameInfo]:
        ...
        try:
            return await self.riot_api_client.get_current_game_info(
                player.summoner_id, player.region
            )
        except PlayerNotInGameError:
            return None
        except Exception as e:
            logger.error(
                f"Error fetching game state for {player.summoner_name}#{player.region}: {e}"
            )
            return None
    ...
    async def _emit_game_state_changed_event(
        ...
        try:
            ...
            # Publish the event
            subject = f"lol.gamestate.{player.region}.{player.summoner_name}"
            await self.message_bus.publish(subject, event.SerializeToString())

            logger.info(
                f"Published game state changed event for {player.summoner_name}#{player.region}: "
                f"{lol_events_pb2.GameStatus.Name(previous_status)} -> {lol_events_pb2.GameStatus.Name(current_status)}"
            )

        except Exception as e:
            logger.error(
                f"Error publishing game state changed event for {player.summoner_name}#{player.region}: {e}"
            )
