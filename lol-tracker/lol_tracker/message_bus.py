...
class NATSMessageBusClient:
    ...
    async def subscribe(self, subject: str, handler) -> None:
        ...
        try:
            ...
            logger.info(f"Creating subscription for subject {subject} with consumer {consumer_name}")
                ...
                try:
                    ...
                    for msg in msgs:
                        try:
                            ...
                            await msg.ack()
                            logger.debug(f"Processed and acknowledged message from {subject}")
                        except Exception as e:
...
class MockMessageBusClient:
    """Mock message bus client for testing."""
    ...
    async def connect(self) -> None:
        ...
        self._connected = True
        logger.info("Mock: Connected to message bus")
    
    async def disconnect(self) -> None:
        """Mock disconnect operation."""
        self._connected = False
        logger.info("Mock: Disconnected from message bus")
    ...
    async def create_streams(self) -> None:
        ...
        logger.info("Mock: Created JetStream streams")
    ...
    async def subscribe(self, subject: str, handler) -> None:
        ...
        logger.debug(f"Mock: Subscribed to {subject}")
    
    async def trigger_message(self, subject: str, data: bytes) -> None:
        """Trigger a message for testing purposes."""
        if subject in self.subscriptions:
            await self.subscriptions[subject](data)
