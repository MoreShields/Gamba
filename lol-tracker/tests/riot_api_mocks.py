...
class RiotAPIMockRouter:
    ...
    def mock_rate_limit_error(
        self, summoner_name: str, region: str = "na1", retry_after: int = 60
    ):
    ...
    def mock_api_error(
        self, summoner_name: str, region: str = "na1", status_code: int = 500
    ):
...
def mock_successful_summoner_lookup(
    summoner_name: str = "TestSummoner", region: str = "na1", **summoner_data
):
    ...
    router.mock_get_summoner_by_name(
        summoner_name=summoner_name, region=region, response_data=response_data
    )
...
def mock_summoner_not_found(
    summoner_name: str = "NonExistentSummoner", region: str = "na1"
):
...
def mock_rate_limit_error(
    summoner_name: str = "TestSummoner", region: str = "na1", retry_after: int = 60
):
...
def mock_api_error(
    summoner_name: str = "TestSummoner", region: str = "na1", status_code: int = 500
):
...
def create_mock_riot_api_client():
    ...
    mock_client.get_summoner_by_name.return_value = (
        RiotAPIMockData.get_summoner_response()
    )
