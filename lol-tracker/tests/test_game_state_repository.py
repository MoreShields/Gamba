...
class TestGameStateRepository:
    ...
    @pytest.mark.asyncio
    async def test_create_game_state_minimal(
        self, db_session: AsyncSession, tracked_player
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_latest_for_player(
        self, db_session: AsyncSession, tracked_player
    ):
    ...
    async def test_get_by_game_id(self, db_session: AsyncSession, tracked_player):
        ...
        state1 = await repo.create(
            tracked_player.id, "IN_GAME", game_id="shared_game_123"
        )
        state2 = await repo.create(player2.id, "IN_GAME", game_id="shared_game_123")
    ...
    async def test_update_game_result(self, db_session: AsyncSession, tracked_player):
        ...
        # Create a game state
        game_state = await repo.create(
            tracked_player.id, "IN_GAME", game_id="result_test_game"
        )
    ...
    @pytest.mark.asyncio
    async def test_update_game_result_auto_end_time(
        self, db_session: AsyncSession, tracked_player
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_recent_games_for_player(
        self, db_session: AsyncSession, tracked_player
    ):
        ...
        for i in range(15):
            state = await repo.create(
                tracked_player.id, "NOT_IN_GAME", game_id=f"game_{i}"
            )
            states.append(state)
    ...
    @pytest.mark.asyncio
    async def test_get_recent_games_with_custom_limit(
        self, db_session: AsyncSession, tracked_player
    ):
        ...
        recent_games = await repo.get_recent_games_for_player(
            tracked_player.id, limit=3
        )

        assert len(recent_games) == 3

    @pytest.mark.asyncio
    async def test_get_recent_games_empty(
        self, db_session: AsyncSession, tracked_player
    ):
    ...
    async def test_get_active_games(self, db_session: AsyncSession, tracked_player):
        ...
        active_state1 = await repo.create(
            tracked_player.id, "IN_GAME", game_id="active_1"
        )
    ...
    async def test_delete_old_states(self, db_session: AsyncSession, tracked_player):
        ...
        for i in range(15):
            state = await repo.create(
                tracked_player.id, "NOT_IN_GAME", game_id=f"old_game_{i}"
            )
        ...
        remaining_states = await get_game_states_for_player(
            db_session, tracked_player.id
        )
    ...
    @pytest.mark.asyncio
    async def test_delete_old_states_keep_all(
        self, db_session: AsyncSession, tracked_player
    ):
        ...
        for i in range(3):
            await repo.create(
                tracked_player.id, "NOT_IN_GAME", game_id=f"keep_game_{i}"
            )
    ...
    @pytest.mark.asyncio
    async def test_delete_old_states_no_states(
        self, db_session: AsyncSession, tracked_player
    ):
    ...
    async def test_game_state_sequence(self, db_session: AsyncSession, tracked_player):
        ...
        # Finish the game
        finished = await repo.create(tracked_player.id, "NOT_IN_GAME")

        # Update the in-game state with results
        await repo.update_game_result(
            in_game.id, won=True, duration_seconds=2100, champion_played="Zed"
        )
        ...
        in_game_state = next(
            state for state in game_states if state.status == "IN_GAME"
        )
